...
def create_project_with_keywords(db: Session, user_id: str, name: str, domain: str, location_code: int, location: str, keywords: list[str]) -> Project:
    ...
    normalized_keywords = list(dict.fromkeys(kw.strip().lower() for kw in keywords if kw.strip()))
    ...
    try:
        ...
        for kw in normalized_keywords:
            db.add(Keyword(projectId=project.id, userId=user_id, keyword=kw, location=location))
        db.commit()

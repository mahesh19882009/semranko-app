...
export default function VerifyMobilePage() {
  ...
  const [maskedMobile, setMaskedMobile] = useState(() => (
    typeof window !== 'undefined' ? sessionStorage.getItem('mobileVerificationMasked') : null
  ));
  ...
  const verify = async (event) => {
    ...
    try {
      ...
      sessionStorage.removeItem('mobileVerificationMasked');
      setVerified(true);
  ...
  const resend = async () => {
    ...
    setLoading(true); setError(''); setMessage('');
    try {
      const result = await resendMobileOtpApi(token, turnstileToken);
      const refreshedMask = result?.data?.masked_mobile;
      if (refreshedMask) {
        setMaskedMobile(refreshedMask);
        sessionStorage.setItem('mobileVerificationMasked', refreshedMask);
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-600">Enter the six-digit OTP sent to {maskedMobile || 'your registered mobile number'}. Email verification is also required.</p>
